...
You will create a master and two tenant MySQL database, build a Spring application, and connect it to the newly created databases.

NOTE: MySQL is licensed with the GPL, so any program binary that you distribute with it must use the GPL, too.
See the
...
If you have Docker installed, it might be useful to run the database as a
...
This example needs the Spring Web Starter, Spring Data JPA, and MySQL Driver dependencies.
The following image shows the Initializr set up for this sample project:

image::images/initializr.png[]

NOTE: The preceding image shows the Initializr with Maven chosen as the build tool.
It also shows values of `com.example.multitenant.mysql` and `multitenant-mysql` as the Group and Artifact, respectively.
You will use those values throughout the rest of this sample.
...
NOTE: This connects to MySQL as `root` and allows access to the user from all hosts.
This is *not the recommended way* for a production server.
...
You can find all the necessary scripts under the `docker/provision/mysql/init` folder.
...
Create a resource file called `src/main/resources/application.yaml`, as the following listing shows:
...
The default for `H2` and other embedded databases is `create-drop`.
For other databases, such as `MySQL`, the default is `none`.

NOTE: It is a good security practice to, after your database is in a production state, set this to `none`, revoke all privileges from the MySQL user connected to the Spring application, and give the MySQL user only `SELECT`, `UPDATE`, `INSERT`, and `DELETE`.
You can read more about this at the end of this guide.

== Create the `@Entity` Model

You need to create the entity model, as the following listing (in `src/main/java/com/example/multitenant/mysql/entity/City.java`) shows:

====
[source,java,tabsize=2]
----
include::src/main/java/com/example/multitenant/mysql/entity/City.java[]
----
====
...
You need to create the repository that holds user records, as the following listing (in `src/main/java/com/example/multitenant/mysql/repository/CityRepository.java`) shows:

====
[source,java,tabsize=2]
----
include::src/main/java/com/example/multitenant/mysql/repository/CityRepository.java[]
----
====

Spring automatically implements this repository interface in a bean that has the same name (with a change in the case -- it is called `cityRepository`).

== Create a Controller

You need to create a controller to handle HTTP requests to your application, as the following listing (in `src/main/java/com/example/multitenant/mysql/controller/CityController.java`) shows:

====
[source,java,tabsize=2]
----
include::src/main/java/com/example/multitenant/mysql/controller/CityController.java[]
----
====
...
The following listing shows the class that Initializr created for this example (in
`src/main/java/com/example/multitenant/mysql/Application.java`):

====
[source,java,tabsize=2]
----
include::src/main/java/com/example/multitenant/mysql/Application.java[]
----
====

For this example, you need not modify the `Application` class.
...
When you run the application, logging output is displayed.
The service should be up and running within a few seconds.

== Test the Application

Now that the application is running, you can test it by using `web browser`, `postman`,  `curl` or some similar tool.
You can access HTTP endpoints that you can test from `swagger-ui` page:
...
[source,sh]
----
$ curl -X POST http://localhost:8080/ -H 'Content-Type: application/json' -H 'X-TenantID: mysql_tenant_1' -d '{"name":"Ankara"}'
----
====
...
Congratulations!
You have just developed a Spring application that is bound to a MySQL database and is ready for production!
